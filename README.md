...
Finally, the robot must be re-spawn in Gazebo (as to get back on its feet). It can be done by either resteeting the Gazebo simulation (CTRL+R in Gazebo) or with the use of a service:
```bash
cd ~/catkin_ws
source devel/setup.bash
